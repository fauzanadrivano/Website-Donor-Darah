...
if($op == 'delete'){
    $id     = $_GET['id'];
    $sql1       = "delete from riwayat_darah_masuk where id = '$id'";
...
if ($op == 'edit') {
    $id             = $_GET['id'];
    $sql1           = "select * from riwayat_darah_masuk where id = '$id'";
    $q1             = mysqli_query($koneksi, $sql1);
...
if(isset($_POST['simpan'])){
    ...
    if($id_posko && $golongan_darah && $kantong && $rumah_sakit && $tanggal){
        if ($op == 'edit') {
            $id = $_GET['id'];
            $sql1 = "UPDATE riwayat_darah_masuk SET id_posko='$id_posko', golongan_darah='$golongan_darah', kantong='$kantong', rumah_sakit='$rumah_sakit', tanggal='$tanggal' WHERE id='$id'";
            $q1 = mysqli_query($koneksi, $sql1);
                ...
                <div class = "card-body">
                    <form action="" method="POST">
                        <table class="table">
                            ...
                            <tbody>
                                    <?php
                                    $sql2   =   "select * from riwayat_darah_masuk order by id desc";
                                    ...
                                    <tr>
                                        ...
                                        <td scope="row">
                                            <a href="riwayat_darah_masuk.php?op=edit&id=<?php echo $id ?>"><button type="button" class="btn btn-warning">Edit</button></a>
                                            <a href="riwayat_darah_masuk.php?op=delete&id=<?php echo $id?>" onclick="return confirm('Hapus Data ?')"><button type="button" class="btn btn-danger">Delete</button></a>            
                                        </td>
                                    </tr>
                                    <?php
                                    }
